...
import pandas as pd
import random
import xml_generator_variable_length
import time
...
gymnasium.envs.register(
    id="Snake-v14",
    entry_point=f"{__name__}:SnakeEnvMujoco",
    max_episode_steps=300,  #25 step = 1 sec
    reward_threshold=1000,
)

design_file = open("/home/riccardo/Desktop/kevin_exp/Coadaptation/Environments/design.txt", "r")
xml_filename = str(design_file.read().replace("\n",""))
xml_file_path = f"/home/riccardo/Desktop/kevin_exp/Coadaptation/Environments/xml_models_variable_lenght/{xml_filename}"
...
class SnakeEnvMujoco(MujocoEnv, utils.EzPickle):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._optimized_params = np.random.uniform(low=0, high=149, size=7)
        ...
        print("xml file from snake env chosen: ", xml_file )

    def control_cost(self, action):
        control_cost = self._ctrl_cost_weight * (np.sum(abs(action))/7)/self.max_ctrl_cost        #normalized
        return control_cost
    ...
    def step(self, action):
        ...
        #goal distance
        x_goal_dist = abs(self.goal_pos[0]-xy_position_after[0])   #should go before get obs
        y_goal_dist = abs(self.goal_pos[1]-xy_position_after[1])
        ...
        self._prev_x_pos = xy_position_after[0]
    ...
    def _get_obs(self):
        ...
        #position = position[3:] #------------------------------------delete xyz in position
        #velocity = velocity[3:]
    ...
    def reset_model(self):
        ...
        #goal_df.to_csv("goal_used", sep=',', header=False, float_format='%.2f', index=False)
...
#--------------------------------------------------------------------------------------------
    ...
    def reset_design(self, vec):
        xml_generator_variable_length.generate_xml(vec)
        xml_filename = "snake_v14"
        param = '_'.join(map(str, vec))
        xml_newfilename = f"{xml_filename}_{param}.xml"
        # print("xml_newfilename",xml_newfilename)
        text_file = open("Environments/design.txt", "w")
        text_file.write(xml_newfilename)
        text_file.close()
        print("sleep for 1 sec bc changing design txt")
        time.sleep(1)
